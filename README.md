...
CLI 클라이언트의 `send_command()` 함수는 서버 연결 실패 시 최대 3회까지 지수 백오프(Exponential Backoff) 방식으로 재시도합니다.

| 시도 | 대기 시간 |
|------|-----------|
| 1차 실패 | 1초 후 재시도 |
| 2차 실패 | 2초 후 재시도 |
| 3차 실패 | 에러 메시지 출력 후 메뉴 복귀 |
...
- **최초 1회 로딩**: 서버는 시작 시점에 `dlopen()`을 사용하여 `libled.so`, `libbuzzor.so` 등 장치 드라이버 라이브러리 파일을 메모리에 한 번만 로드합니다.
- **업그레이드 반영**: 따라서 서버 동작 중에 `.so` 파일만 교체하더라도 실행 중인 서버에 실시간으로 반영되지 않으며, 서버를 재시작해야 새로운 라이브러리가 메모리에 로드되어 반영됩니다.
