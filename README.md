...
| 항목 | 현재 README 기준 |
|------|------------------|
| 공통 헤더 경로 | 현재 레포 구조에서는 `lib/rpi_common.h`를 사용합니다. `include/rpi_common.h` 구조로 바꾸려면 코드와 Makefile 이동 작업이 함께 필요합니다. |
| CLI 7세그먼트 입력 | CLI 7세그먼트 메뉴는 입력 문자열을 `segment/<입력>`으로 전달하므로 `start`, `off`, `0~9`, `show/0~9` 형태로 시연할 수 있습니다. |
| 1자리 7세그먼트 `00` 해석 | 장치가 1자리 7세그먼트이므로 `00`은 별도 두 자리 표시가 아니라 단일 숫자 `0`으로 설명합니다. 시연 입력도 `0` 또는 `show/0`을 기준으로 합니다. |
| 클라이언트 시그널 처리 | 현재 코드는 `SIGINT`에서 자동 제어 중지 요청과 소켓 정리를 수행하고, `SIGTSTP`/`SIGQUIT`은 무시합니다. 더 엄밀한 POSIX 비동기 안전성을 요구하면 `sigaction()` + `volatile sig_atomic_t` 플래그 방식으로 개선해야 합니다. |
| 연결 실패 UX | 현재는 서버 접속 실패 시 재시도 백오프가 없으므로, 보완 지점으로 기록합니다. |
...
| Photoresistor | 메뉴 번호 `1~4` | `/pr`, `/pr/digital`, `/pr/auto/start`, `/pr/auto/stop` |

7세그먼트는 1자리 장치이므로 두 자리 값 `00`을 표시하지 않습니다. 카운트다운 또는 단발 표시에서 0을 다룰 때는 단일 숫자 `0`으로 입력하고 문서화합니다.
...
| 시그널 처리 안전성 | 현재 구현은 `SIGINT` 핸들러에서 정리 작업을 수행합니다. 평가자가 비동기 안전성을 엄격히 보면 `sigaction()`과 `sig_atomic_t` 플래그 기반으로 개선하는 편이 더 안전합니다. |
| CLI 7세그먼트 입력 검증 | `start`, `off`, `0~9`, `show/0~9` 시연은 가능하지만 잘못된 문자열 검증은 서버/클라이언트 양쪽에서 더 강화할 수 있습니다. |
